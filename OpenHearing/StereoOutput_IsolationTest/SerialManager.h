...
extern bool activateTone(bool,int chan);
...
void SerialManager::printHelp(void) {  
  ...
  Serial.println("   a/A: TONE: Activate (a) or Mute (A) tone on all channels.");
  Serial.print(  "   1,2,3,4: TONE : Activate tone on channels 1-4 (is_active = [");
  for (int Ichan=0; Ichan < myState.n_channels; Ichan++) { Serial.print(String(myState.is_tone_active[Ichan]) + ", "); };  Serial.println("])");
  Serial.print(  "   !,@,#,$: TONE : De-activate tone on channels 1-4");
...
bool SerialManager::processCharacter(char c) { //this is called by SerialManagerBase.respondToByte(char c)
  ...
  switch (c) {
    ...
    case 'a':
      activateTone(true,-1);
      Serial.println("Activating the tone on all channels: loudness = " + String(myState.tone_dBFS,1) + " dBFS");
      break;
    case 'A':
      activateTone(false,-1);
      Serial.println("Muting the tone on all channels...");
      break;
    case '1': case '2': case '3': case '4':
      { 
        int Ichan=(int(c)-int('0')); 
        activateTone(true,Ichan-1);
        Serial.println("Activating the tone on channel " + String(Ichan) + ": loudness = " + String(myState.tone_dBFS,1) + " dBFS");
      }
      break;
    case '!': case '@': case '#': case '$':
      { int Ichan=99;
        ...
        activateTone(false,Ichan-1);
        Serial.println("Muting the tone on channel " + String(Ichan));
      }
      break;
