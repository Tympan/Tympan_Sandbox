...
class State : public TympanStateBase_UI { // look in TympanStateBase or TympanStateBase_UI for more state variables and helpful methods!!
  public:
    ...
    float output_dacGain_dB = 0.0;  //gain of the hardware headphone amplifier in the AIC.  Leave at 0.0 unlesl you have a good reason.

    //tone parameters
    float tone_Hz = 1000.0;         //freuqency of tone.  any value less than Nyquist
    float tone_dBFS = -8*3.0;        //loudness of tone.  any value less than 0.0 dB
    const int n_channels = 4;
    bool is_tone_active[4] = {true, false, false, false};    //the code will flip this to true whenever we want to hear the tone
