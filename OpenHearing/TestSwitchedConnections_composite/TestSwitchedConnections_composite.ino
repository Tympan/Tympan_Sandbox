/*
*   TestSwtichedAudioInstances_composite
*
...
std::vector<AudioStreamComposite_F32 *> allAudioPaths;            //fill it later
...
void createMyAudioPathObjects(AudioSettings_F32 &_audio_settings) {
  ...
  allAudioPaths.push_back( new AudioPath_PassThruGain_Analog(audio_settings) );

  //Add Audio Path: Audio pass-thru with gain (PDM mics)
  allAudioPaths.push_back( new AudioPath_PassThruGain_PDM(audio_settings) );

  //Add Audio Path: Sine wave generator with FFT analysis of the input
  allAudioPaths.push_back( new AudioPath_Sine_wFFT(audio_settings) );

  //Add Audio Path: Add yours here!
  //allAudioPaths.push_back( new myAudioPathClassName(audio_settings) ); 

  // Give pointers to the hardware to each of the Audio Path objects
  for (auto & audioPath : allAudioPaths) { audioPath->setTympanHardware(tympan_ptr, shield_ptr); } //iterate over each audio object in the audioObjects vector
}
...
void connectAllAudioObjects(void) {
  //Connect each AudioPath to the Tympan's inputs
  for (int Ipath=0; Ipath < (int)allAudioPaths.size(); Ipath++) {  //loop over each AudioPath
    #if USE_FOUR_CHANNELS
      patchCords.push_back( new AudioConnection_F32(*audioInput, EarpieceShield::PDM_LEFT_FRONT,  *allAudioPaths[Ipath], 0) );
      patchCords.push_back( new AudioConnection_F32(*audioInput, EarpieceShield::PDM_LEFT_REAR,   *allAudioPaths[Ipath], 1) );
      patchCords.push_back( new AudioConnection_F32(*audioInput, EarpieceShield::PDM_RIGHT_FRONT, *allAudioPaths[Ipath], 2) );
      patchCords.push_back( new AudioConnection_F32(*audioInput, EarpieceShield::PDM_RIGHT_REAR,  *allAudioPaths[Ipath], 3) );
    #else
      patchCords.push_back( new AudioConnection_F32(*audioInput, 0,  *allAudioPaths[Ipath], 0) );
      patchCords.push_back( new AudioConnection_F32(*audioInput, 1,  *allAudioPaths[Ipath], 1) );
    #endif
  }

  //Connect each AudioPath to each of the output mixers
  for (int Imixer=0; Imixer < (int)allOutputMixers.size(); Imixer++) {  //loop over each output mixer (which will be each output channel)
    for (int Ipath=0; Ipath < (int)allAudioPaths.size(); Ipath++) {  //loop over each AudioPath and connect it to each output mixer
      ...
      patchCords.push_back( new AudioConnection_F32(*allAudioPaths[Ipath], path_output_channel, *allOutputMixers[Imixer], mixer_input_chanenl) );
    }
  }
...
int activateOneAudioPath(int index) {
  //first, de-activate all the known audio paths
  deactivateAllAudioPaths();
  
  //then, if the given index is valid, activate that one single audio path
  if ((index >= 0) && (index < (int)allAudioPaths.size())) {
    bool activate = true;        //let's command it to activate this audio path
    bool alsoDoMoreSetup = true; //set this to true to allow this one AudioPath to setup the Tympan hardware how it likes
    bool is_active = allAudioPaths[index]->setActive(activate, alsoDoMoreSetup);
    if (is_active) activeAudioPathIndex = index;
  }
  return activeAudioPathIndex;
}

AudioStreamComposite_F32 *getActiveAudioPath(void) {
  if ((activeAudioPathIndex >= 0) && (activeAudioPathIndex < (int)allAudioPaths.size())) {
...
void setup() {
  ...
  Serial.println("setup: createAllAudioObjects(audio_settings)...");
  ...
  Serial.println("setup: connectAllAudioObjects...");
  connectAllAudioObjects();               //connect all of the audio and audiopath objects
  Serial.println("setup: deactivateAllAudioPaths...");
  ...
  Serial.println("setup: AudioMemory_F32...");
  AudioMemory_F32(180,audio_settings); //allocate audio memory...make big if doing the long FFTs in AudioPath_Sine_wFFT

  //configure the Tympan
  Serial.println("setup: enabling myTympan...");
  ...
  if (0) {
    Serial.println("setup: activating an audiopath..");
    activateOneAudioPath(1-1);                //must be after the Tympan and Earpiece shield are enabled or else the setupHardware() method within won't work.
  }
...
void loop() {
  ...
  if (enable_printCPUandMemory) myTympan.printCPUandMemory(millis(),1000); //update every 1000 msec
