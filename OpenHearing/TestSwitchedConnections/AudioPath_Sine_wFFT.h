...
class AudioPath_Sine_wFFT : public AudioPath_Base {
  public:
    ...
    //setActive: extend the default setAcive() method from AudioPath_Base so that it starts/stops the queues
    virtual bool setActive(bool _active) {
      bool active = AudioPath_Base::setActive(_active);
      if (active) { 
        enableTone(true);
        beginRecording(); 
      } else { 
  ...
  protected:
    ...
    const int               Nfft = 2*4096;            //requires it to be poewr of 2 and requires it to be an integer multiple of the length of the samples in an audio block
