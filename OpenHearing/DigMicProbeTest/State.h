...
class State : public TympanStateBase_UI { // look in TympanStateBase or TympanStateBase_UI for more state variables and helpful methods!!
  public:
    ...
    float chirp_amp_dBFS = -20.0;   // limit to less than 0.0 dB
    
    //controls for chirp and chirp+SD modes
    enum AUTO_CHIRPSD_STATES {DISABLED=0,WAIT_START_CHIRP,WAIT_END_CHIRP,WAIT_STOP_SD};
    int auto_chirpsd_state = DISABLED;
    float auto_SD_start_stop_delay_sec = 0.250; //when linking SD to the chirp, how much time before/after chirp to start/stop SD?
    bool chirp_start_armed = false;  //DON'T CHANGE THIS
    unsigned long start_chirp_at_millis = 0;   //DON'T CHANGE THIS.  main loop() will start the chirp at (or after) this time
