...
{
  //Serial.println("BLE_Stuff: connect_callback: starting...");
  ...
  bleConnected = true;
  Serial.print("BLE_Stuff: connect_callback: Connected to "); Serial.print(central_name);
...
{
  //Serial.println("BLE_Stuff: disconnect_callback: starting...");
  (void) conn_handle;
  (void) reason;
  bleConnected = false;
  Serial.println(); Serial.print("disconnect_callback: Disconnected, reason = 0x"); Serial.print(reason, HEX);
  Serial.print(", bleConnected = "); Serial.println(bleConnected);
}
...
int BLEevent(BLEUart *bleuart_ptr, HardwareSerial *serial_ptr) {
  int success = -1;
  if(bleuart_ptr->available()) {
    Serial.print("BLEevent: bleuart_ptr->available()) = "); Serial.println(bleuart_ptr->available());
    success = 0;
    while (bleuart_ptr->available()) serial_ptr->write(bleuart_ptr->read());
  }
  return success;
}

void serialEvent(HardwareSerial *serial_ptr) { //for the nRF firmware, service any messages coming in the serial port from the Tympan
    //interpret received characters as part of AT command set
    if(serial_ptr->available()) {
      Serial.print("BLEevent: serial_ptr->available()) = "); Serial.println(serial_ptr->available());
      while (serial_ptr->available()) AT_interpreter.processSerialCharacter(serial_ptr->read());
    }
 }
