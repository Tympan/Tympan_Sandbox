...
#include "test_gha.h"          //must be before the "AudioEffectXXXXX.h"
#include "AudioEffectBTNRH.h"  //must be after the "test_xxxx.h"
...
AudioEffectBTNRH    BTNRH_effect(audio_settings);
AudioOutputI2S_F32  audio_out(audio_settings);

//connect the audio objects
AudioConnection_F32 patchCord1(audio_in, 0, BTNRH_effect, 0);  //left input
AudioConnection_F32 patchCord3(BTNRH_effect, 0, audio_out, 0); //left output
AudioConnection_F32 patchCord4(BTNRH_effect, 0, audio_out, 1); //right output same as left
...
void setup() { //this runs once at startup
    ...
    delay(1000);
    myTympan.print("CHAPRO for Tympan [test_gha]: setup():...");
    ...
    BTNRH_effect.set_CHA_PTR(cp);     //in AudioEffectBTNRH.h
