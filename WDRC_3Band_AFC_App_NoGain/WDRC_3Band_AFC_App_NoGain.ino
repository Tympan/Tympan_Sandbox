...
const float input_gain_dB = 0.0f; //gain on the microphone...set to 5 dB for earpieces!
...
TympanPins                    tympPins(TYMPAN_REV_D2);        //TYMPAN_REV_C or TYMPAN_REV_D (or D1, D2, or D3)
...
SerialManager serialManager(audioHardware,N_CHAN_MAX,expCompLim,ampSweepTester,freqSweepTester,freqSweepTester_FIR,feedbackCancel);
...
int current_dsl_config = DSL_PRESET_A; //used to select one of the configurations above for startup
...
//    case (DSL_NORMAL):
//      if (s) BOTH_SERIAL.println("incrementDSLConfiguration: changing to NORMAL dsl configuration");
//      setupFromDSLandGHAandAFC(dsl, gha, afc, N_CHAN_MAX, audio_settings);  break;
//    case (DSL_FULLON):
//      if (s) BOTH_SERIAL.println("incrementDSLConfiguration: changing to FULL-ON dsl configuration");
...
void printGainSettings(void) {
  ...
  BOTH_SERIAL.print("Vol Knob = "); BOTH_SERIAL.print(vol_knob_gain_dB,1);
  BOTH_SERIAL.print(", Input PGA = "); BOTH_SERIAL.print(input_gain_dB,1);
  BOTH_SERIAL.print(", Per-Channel = ");
  for (int i=0; i<N_CHAN; i++) {
    BOTH_SERIAL.print(expCompLim[i].getGain_dB()-vol_knob_gain_dB,1);
    BOTH_SERIAL.print(", ");
  }
  BOTH_SERIAL.println();
}
...
void printAveSignalLevelsMessage(Stream *s, bool as_dBSPL) {
  ...
  for (int i=0; i<N_CHAN; i++) { BOTH_SERIAL.print(aveSignalLevels_dBFS[i]+offset_dB,1);  BOTH_SERIAL.print(", ");  }
  BOTH_SERIAL.println();
}
